...
/*        app_->register_message_handler(vsomeip::ANY_SERVICE,
        ...
        app_->register_message_handler(service_id,
                service_instance_id, service_method_id_targetpointnavi,
                std::bind(&test_client::on_message_cbk, this,
                        std::placeholders::_1));
            ...
            vsomeip::instance_t _instance, bool _is_available)
    {
                ...
                && _is_available)
        {
            ...
            rq->set_method(service_method_id_targetpointnavi);        
//            rq->set_method(service_method_id);
            ...
            TargetPointNaviType tar_point;
            tar_point.MissionID = 16;
            tar_point.MissionPoints[1].Heading = 123;
            tar_point.MissionPoints[1].Latitude = 456;
            tar_point.MissionPoints[1].Longitude = 789;
            tar_point.MissionPoints[1].Property = 010;


            std::vector<vsomeip::byte_t> res_data(reinterpret_cast<const uint8_t *>(&tar_point),
                                                   reinterpret_cast<const uint8_t *>(&tar_point) + sizeof(TargetPointNaviType));
...
//            std::vector<vsomeip::byte_t> pl_data(std::begin(str), std::end(str));
            ...
            // LOG_INF("Sending: TargetPointNaviType");
            app_->send(rq);
        }
    }
